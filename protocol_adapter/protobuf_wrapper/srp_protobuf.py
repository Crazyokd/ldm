...
from protocol_adapter.huawei_model.const import HMIUserSetState
from protocol_adapter.protobuf_wrapper import main_pb2
from protocol_adapter.protobuf_wrapper import frame as srp_frame
...
class SrpProtobuf:
    ...
    def move_to_station(
        self, seq, no, station_id, avoid_policy=main_pb2.MovementTask.OBSTACLE_AVOID_WAIT
    ):
    ...
    def start_location(self, seq, absolute_location=False):
        _logger.info(f'start location {seq}')
        self._send_command_msg(seq, main_pb2.CMD_START_LOCATION, paramBool=absolute_location)

    def switch_map(self, seq, map_name, x, y, angle, absolute_location=False):
        _logger.info(f'switch map seq: {seq}, new map: {map_name}')
        pose = main_pb2.Pose(x=x, y=y, yaw=int(math.radians(angle) * 1000))
        self._send_command_msg(
            seq,
    ...
    def set_initial_pose(self, seq, x, y, angle, x_factor, y_factor):
        pose = main_pb2.Pose(x=x, y=y, yaw=int(math.radians(angle) * 1000))
        self._send_command_msg(
            seq,
    ...
    def _send_command_msg(
        ...
        message = main_pb2.Message(
            type=main_pb2.MSG_COMMAND, seq=seq, session_id=self._session_id, command=command
        )
    ...
    def _sendRequestMsg(
        ...
        message = main_pb2.Message(
            type=main_pb2.MSG_REQUEST, seq=seq, session_id=self._session_id, request=request
        )
    ...
    def _send_response_msg(self, response_type, seq, notify_type):
        ...
        response = main_pb2.Response(
            response_type=response_type, notify_response=notification_response
        )
        message = main_pb2.Message(
            type=main_pb2.MSG_RESPONSE, seq=seq, session_id=self._session_id, response=response
        )
    ...
    def _handleRecvResponseMsg(self, msg):
        ...
        if response.result.result_state in (
            ...
            self._response_callback(
                msg.seq, response.response_type, False, result_code=response.result.result_code
            )
    ...
    def _ack_notification(self, message):
        ...
        self._send_response_msg(
            main_pb2.Response.RESPONSE_NOTIFY, message.seq, message.notification.notify_type
        )
